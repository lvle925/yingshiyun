...
def detect_illegal_content(prompt: str) -> bool:
    """检测是否包含违法内容 - 关键词快速检测（作为LLM意图识别的补充）"""
    illegal_keywords = [
        "破解", "黑客", "盗取", "窃取", "侵入", "攻击", "入侵",
        "贩卖", "走私", "诈骗", "洗钱", "赌博", "毒品", "偷盗",
        "非法", "违法", "犯罪", "盗号", "盗窃", "伤害他人", "报复", "陷害",
        "杀人", "杀害", "谋杀", "自杀", "杀死", "伤人", "绑架", "勒索",
        "爆炸", "制毒", "贩毒", "吸毒", "强奸", "性侵", "猥亵", "拐卖",
        "恐怖", "暴力", "械斗", "血腥", "虐待", "折磨"
    ]
...
def detect_reincarnation_query(prompt: str) -> bool:
    """检测是否询问来世/下辈子等超出服务范围的问题"""
    keywords = [
        "下辈子",
        "下世",
        "下一世",
...
def detect_finance_investment_query(prompt: str) -> bool:
    """
    检测金融/投资/彩票类问题，提前固定回复。
    """
    keywords = [
        # 金融/投资通用
